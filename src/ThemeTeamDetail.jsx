...
export default function ThemeTeamDetail() {
  ...
  // Team name mappings (URL format to database format)
  const teamMappings = {
    "49ers": "49ers",
    "bears": "Bears",
    "bengals": "Bengals",
  ...
  // Program name mappings (URL format to database format)
  ...
  // Organize players by position and get top 5 per position
  ...
  const organizedPlayers = organizePlayersByPosition(filteredPlayers);

  // Calculate team OVR (sum of highest OVR at each position)
    ...
    Object.values(organizedPlayers).forEach(group => {
      Object.values(group).forEach(playersList => {
        if (playersList.length > 0) {
          // Add the highest OVR (first player after sorting)
  ...
  const getDisplayName = () => {
    if (type === "team") {
      return `Theme Team: ${teamMappings[name]}`;
    }
    if (type === "division") {
      const divisionName = name.split('-').map(w => w.toUpperCase() === 'AFC' || w.toUpperCase() === 'NFC' ? w.toUpperCase() : w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
      return `Theme Team: ${divisionName}`;
    }
    if (type === "conference") {
      return `Theme Team: ${name.toUpperCase()}`;
    }
    if (type === "program") {
      return `Theme Team: ${programMappings[name]}`;
    }
    return "Theme Team";
  };
